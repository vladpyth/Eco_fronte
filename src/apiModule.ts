export type ApiModule = "rhzo" | "roo" | "roio" | "ponoinput";
...
/** /api-poo → РОО (8081), /api-ponod → РОИО (8082), /api-ponoinput → внесение (8083), /api → РХЗО (8080). */
export function resolveApiPath(path: string): string {
  ...
  if (path.startsWith("/api/")) {
    ...
    if (currentModule === "ponoinput") return `/api-ponoinput${path.slice(4)}`;
  }
  return path;
}
